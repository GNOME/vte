...
class BoolOption {
public:
        BoolOption() : m_value(false), m_set(false) { }
        ~BoolOption() { }

        bool get() const { return m_set ? m_value : false; }
        void set(bool b) { m_value = b; m_set = true; }
...
class ColorOption {
public:
        ColorOption() : m_set(false) { m_color = { 0, 0, 0, 0 }; }
        ~ColorOption() { }

        GdkRGBA const& get() const { return m_color; }
        void set(GdkRGBA const& c) { m_color = c; m_set = true; }
        void set(char const* s) { GdkRGBA c; if (gdk_rgba_parse(&c, s)) set(c); else reset(); }
        void reset() { m_set = false; }
        bool is_set() const { return m_set; }
private:
        GdkRGBA m_color;
...
class StringOption {
public:
        StringOption() : m_string(nullptr) { }
        ~StringOption() { reset(); }

        char const* get() const { return m_string; }
        void set(char const* s) { reset(); m_string = g_strdup(s); }
        void reset() { g_clear_pointer(&m_string, g_free); }
        bool is_set() const { return m_string != nullptr; }
private:
        char* m_string;
};
