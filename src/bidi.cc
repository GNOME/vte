 ...
 *
 * FIXMEegmont This is no longer true, we store vteunistr with combining accents.
 * FIXMEegmont This should have a wrapper method in RingView!!!
 */
vteunistr
 ...
 * in place.
 */
void BidiRunner::explicit_line(vte::grid::row_t row, bool rtl, bool do_shaping)
{
        int i;
        ...
        if (G_LIKELY (!rtl)) {
                if (!do_shaping || !needs_shaping(row)) {
...
void BidiRunner::explicit_paragraph(vte::grid::row_t start, vte::grid::row_t end, bool rtl, bool do_shaping)
{
        for (; start < end; start++) {
 ...
 * Returns success. */
bool BidiRunner::implicit_paragraph(vte::grid::row_t start, vte::grid::row_t end, bool do_shaping)
{
        const VteCell *cell;
