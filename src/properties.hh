...
enum class Flags {
        ...
        // private, systemd properties only
        SYSTEMD_START = 1u << 2, // start property
        SYSTEMD_END = 1u << 3, // end property
        SYSTEMD = SYSTEMD_START | SYSTEMD_END,
};
...
inline constexpr auto const k_max_systemd_string_len = 255u;

// On return, @pos will point to the first character not parsed yet.
inline std::optional<char8_t>
parse_systemd_string_escape(std::string_view const& str,
                            std::string_view::size_type& pos) noexcept
{
        auto estr = str.substr(pos, 4);
        pos += 4;
...
parse_systemd_property_string(std::string_view str)
{
        static constinit auto const needle = "\\"sv;
        ...
        while (str.size() != 0) {
                ...
                auto ec = parse_systemd_string_escape(str, run);
                ok = bool(ec);
                if (!ok)
                        break;
        ...
        if (ok &&
            g_utf8_strlen(unescaped.c_str(), unescaped.size()) <= k_max_systemd_string_len)
                return unescaped;
...
inline std::optional<std::string>
unparse_systemd_property_string(std::string_view str)
{
        static constinit auto const needle = ";\\"sv;
        ...
        while (str.size() != 0) {
                ...
                switch (str[run]) {
                case ';':  ostr.append("\\x3b"sv);  break;
                case '\\': ostr.append("\\x5c"sv); break;
                default: __builtin_unreachable(); break;
                }
...
inline std::optional<Value>
parse_systemd_property_uuid(std::string_view str)
{
        try {
                return vte::uuid{str, vte::uuid::format::ANY_ID128};
        } catch (...) {
                return std::nullopt;
        }
}
...
inline std::optional<Value>
parse_systemd_property_value(Type type,
                             std::string_view const& value) noexcept
{
        switch (type) {
        case Type::STRING:
                return impl::parse_systemd_property_string(value);
        case Type::UUID:
                return impl::parse_systemd_property_uuid(value);
        default:
                return parse_termprop_value(type, value);
        }
}

inline std::optional<std::string>
unparse_systemd_prop_value(Type type,
                           Value const& value)
{
        switch (type) {
        case Type::STRING:
                if (std::holds_alternative<std::string>(value)) {
                        return impl::unparse_systemd_property_string(std::get<std::string>(value));
                }
                break;
        default:
                break;
        }
        return unparse_termprop_value(type, value);
}

inline std::optional<vte::uuid>
parse_systemd_context_id(std::string_view str)
{
        try {
                return std::make_optional<vte::uuid>(str, vte::uuid::format::ANY_ID128);
        } catch (...) {
        }
        ...
        // This is a v4 UUID used as a namespace, see
        // https://datatracker.ietf.org/doc/html/rfc9562#section-6.6
        static constinit auto systemd_namespace_uuid = VTE_DEFINE_UUID(89b6fe6d, a8d2, 4fda, aaaf, 678fc903276f);

        return std::make_optional<vte::uuid>(vte::uuid_v5,
                                             systemd_namespace_uuid,
                                             str);
}
